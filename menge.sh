#!/bin/sh
# menge: Read the Word of God from the German Menge Bible from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data menge.tsv | awk -v cmd=list "$(get_data menge.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data menge.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data menge.awk)" | ${PAGER}
	done
	exit 0
fi

get_data menge.tsv 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data menge.awk)" 2>/dev/null | ${PAGER}
